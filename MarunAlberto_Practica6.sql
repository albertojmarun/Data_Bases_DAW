...
WHERE (SELECT COUNT(EMPLOYEE_ID) FROM EMPLOYEES emp WHERE SALARY > 2500 AND dep.department_id = emp.department_id) > 0
...
SELECT EMP.FIRST_NAME, (SELECT DEP.DEPARTMENT_ID FROM DEPARTMENTS DEP WHERE EMP.DEPARTMENT_ID = DEP.DEPARTMENT_ID) AS ID_DEPARTAMENTO
FROM EMPLOYEES EMP;
